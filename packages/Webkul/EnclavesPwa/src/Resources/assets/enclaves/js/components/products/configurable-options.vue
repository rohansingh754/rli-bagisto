<template>
    <div class="attributes mt-8">
        <div :key="index" v-for='(attribute, index) in childAttributes' class="attribute" :class="[attribute.swatch_type, errors.has('super_attribute[' + attribute.id + ']') ? 'has-error' : '']">
            ...
            <h2 class="text-[15px] font-medium text-dark">{{ attribute.label }}</h2>
        ...
        methods: {
            ...
            reloadPrice () {
                ...
                if (this.childAttributes.length == selectedOptionCount) {
                    ...
                    EventBus.$emit('update-product-attributes', this.simpleProduct);
                    EventBus.$emit('update-product-images', this.config.variant_images[this.simpleProduct]);
